...
--Smooth Dashing Movement
local function move(playerID, xDir, yDir, remainingDist)
	local player = Player(playerID)
	local dist = math.floor((remainingDist/2)+0.5); --Lua Rounding
	local actualDist = 0;

	if not player then return end --Safety Check Player Reference

	--Vector Location Calc
	local newPos = player:getPosition()
	newPos.x = newPos.x + (xDir * dist)
	newPos.y = newPos.y + (yDir * dist)

	--Apply Teleportation
	player:teleportTo(newPos)

	--Recursion
	if (remainingDist - dist > 0) then
		addEvent(move, 15, playerID, xDir, yDir, remainingDist - dist)
	end
end

--Find end of Dash
local function calculateDist(playerID, xDir, yDir, dist) 
	local player = Player(playerID)
	...
	for i=0,dist-1 do
		...
		tempPos.x = tempPos.x + xDir
		tempPos.y = tempPos.y + yDir

		if Tile(tempPos):hasFlag(TILESTATE_BLOCKSOLID) then 
...
--Player Dash Method
function Player:onDash(player)
	local dir = player:getDirection()
	local dist = 6 --Based on Question 6 Video
	local playerID = player:getId()
	
	--Setup Vector Direction
	local xDir = -5;
	local yDir = -5;

	if dir % 2 == 0 then
		--Y
		if dir == 0 then yDir = -1
		elseif dir == 2 then yDir = 1 end
		xDir = 0
	else
		--X
		if dir == 1 then xDir = 1
		elseif dir == 3 then xDir = -1 end
		yDir = 0
	end
	
	local magicNumber = -5 --Normalized Directions, any value beyond (-1, 0, 1) would work
	if xDir == magicNumber or yDir == magicNumber then
		print("[ERROR: player.lua:onDash] - Improper Directional Calculation") --Based on TFS error documentation
		return
	end
	--------------------------

	--Animation
	move(playerID, xDir, yDir, calculateDist(playerID, xDir, yDir, dist))
	-----------
end
