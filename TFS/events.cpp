...
bool Events::load()
{
	...
	for (auto eventNode : doc.child("events").children()) {
		...
		if (className == "Creature") {
			...
			} else if (methodName == "onDash") {
				info.playerOnDash = event;
			} else {
...
void Events::eventPlayerDash(Player* player)
{
	//onDash(player)
	if (info.playerOnDash == -1) {
		std::cout << "[Error - Events::eventPlayerDash] event unassigned" << std::endl;
		return;
	}

	if (!scriptInterface.reserveScriptEnv()) {
		std::cout << "[Error - Events::eventPlayerDash] Call stack overflow" << std::endl;
		return;
	}

	ScriptEnvironment* env = scriptInterface.getScriptEnv();
	env->setScriptId(info.playerOnDash, &scriptInterface);

	lua_State* L = scriptInterface.getLuaState();
	scriptInterface.pushFunction(info.playerOnDash);
